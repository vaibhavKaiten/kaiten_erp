            ...
            if (frm.doc.assigned_vendor) {
                return {
                    ...
                    filters: {
                        ...
                        'doctype_name': 'Structure Mounting'
                    }
                };
            }
            return {};
...
async function capture_workflow_gps_structure_mounting(frm) {
    ...
    try {
        ...
        await set_gps_value_if_exists_structure_mounting(frm, 'gps_latitude', latitude);
        await set_gps_value_if_exists_structure_mounting(frm, 'custom_gps_latitude', latitude);
        await set_gps_value_if_exists_structure_mounting(frm, 'gps_longitude', longitude);
        await set_gps_value_if_exists_structure_mounting(frm, 'custom_gps_longitude', longitude);
        await set_gps_value_if_exists_structure_mounting(frm, 'gps_location', location);
        await set_gps_value_if_exists_structure_mounting(frm, 'custom_gps_location', location);
        await set_gps_value_if_exists_structure_mounting(frm, 'gps_map_link', map_link);
        await set_gps_value_if_exists_structure_mounting(frm, 'custom_gps_map_link', map_link);
    } catch (error) {
        frappe.throw(get_workflow_gps_error_message_structure_mounting(error));
    } finally {
        frappe.dom.unfreeze();
    }
}
...
async function set_gps_value_if_exists_structure_mounting(frm, fieldname, value) {
    if (frm.fields_dict && frm.fields_dict[fieldname]) {
        await frm.set_value(fieldname, value);
    }
}

function get_workflow_gps_error_message_structure_mounting(error) {
