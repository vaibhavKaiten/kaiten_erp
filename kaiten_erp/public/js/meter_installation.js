...
frappe.ui.form.on('Meter Installation', {
    refresh: function (frm) {
        setup_custom_location_log_link_formatter_meter_installation(frm);
        setup_assigned_vendor_filter_meter_installation(frm);
            ...
            if (frm.doc.assigned_vendor) {
                return {
                    ...
                    filters: {
                        ...
                        'doctype_name': 'Meter Installation'
                    }
                };
            }
            return {};
    ...
    before_workflow_action: async function(frm) {
        await capture_workflow_gps_meter_installation(frm);
    }
});

async function capture_workflow_gps_meter_installation(frm) {
    ...
    try {
        ...
        await set_gps_value_if_exists_meter_installation(frm, 'gps_latitude', latitude);
        await set_gps_value_if_exists_meter_installation(frm, 'custom_gps_latitude', latitude);
        await set_gps_value_if_exists_meter_installation(frm, 'gps_longitude', longitude);
        await set_gps_value_if_exists_meter_installation(frm, 'custom_gps_longitude', longitude);
        await set_gps_value_if_exists_meter_installation(frm, 'gps_location', location);
        await set_gps_value_if_exists_meter_installation(frm, 'custom_gps_location', location);
        await set_gps_value_if_exists_meter_installation(frm, 'gps_map_link', map_link);
        await set_gps_value_if_exists_meter_installation(frm, 'custom_gps_map_link', map_link);
    } catch (error) {
        frappe.throw(get_workflow_gps_error_message_meter_installation(error));
    } finally {
        frappe.dom.unfreeze();
    }
}

function setup_assigned_vendor_filter_meter_installation(frm) {
...
function setup_custom_location_log_link_formatter_meter_installation(frm) {
...
async function set_gps_value_if_exists_meter_installation(frm, fieldname, value) {
    if (frm.fields_dict && frm.fields_dict[fieldname]) {
        await frm.set_value(fieldname, value);
    }
}

function get_workflow_gps_error_message_meter_installation(error) {
