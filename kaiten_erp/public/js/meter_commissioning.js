...
frappe.ui.form.on('Meter Commissioning', {
    refresh: function (frm) {
        setup_custom_location_log_link_formatter_meter_commissioning(frm);
        setup_assigned_vendor_filter_meter_commissioning(frm);
            ...
            if (frm.doc.assigned_vendor) {
                return {
                    ...
                    filters: {
                        ...
                        'doctype_name': 'Meter Commissioning'
                    }
                };
            }
            return {};
    ...
    before_workflow_action: async function(frm) {
        await capture_workflow_gps_meter_commissioning(frm);
    }
});

async function capture_workflow_gps_meter_commissioning(frm) {
    ...
    try {
        ...
        await set_gps_value_if_exists_meter_commissioning(frm, 'gps_latitude', latitude);
        await set_gps_value_if_exists_meter_commissioning(frm, 'custom_gps_latitude', latitude);
        await set_gps_value_if_exists_meter_commissioning(frm, 'gps_longitude', longitude);
        await set_gps_value_if_exists_meter_commissioning(frm, 'custom_gps_longitude', longitude);
        await set_gps_value_if_exists_meter_commissioning(frm, 'gps_location', location);
        await set_gps_value_if_exists_meter_commissioning(frm, 'custom_gps_location', location);
        await set_gps_value_if_exists_meter_commissioning(frm, 'gps_map_link', map_link);
        await set_gps_value_if_exists_meter_commissioning(frm, 'custom_gps_map_link', map_link);
    } catch (error) {
        frappe.throw(get_workflow_gps_error_message_meter_commissioning(error));
    } finally {
        frappe.dom.unfreeze();
    }
}

function setup_assigned_vendor_filter_meter_commissioning(frm) {
...
async function set_gps_value_if_exists_meter_commissioning(frm, fieldname, value) {
    if (frm.fields_dict && frm.fields_dict[fieldname]) {
        await frm.set_value(fieldname, value);
    }
}

function get_workflow_gps_error_message_meter_commissioning(error) {
