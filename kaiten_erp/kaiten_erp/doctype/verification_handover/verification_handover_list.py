...
"""
Verification Handover – List View Visibility (Fresh & Strict)
...
def get_permission_query_conditions(user=None):
    """
    List view filter for Verification Handover
    ...
    if is_vendor_manager:
        return f"""
            `tabVerification Handover`.name IN ({doc_list})
            AND `tabVerification Handover`.assigned_vendor = {frappe.db.escape(supplier)}
        """
