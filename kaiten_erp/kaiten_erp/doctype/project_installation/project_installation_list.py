...
"""
Project Installation – List View Visibility (Fresh & Strict)
...
def get_permission_query_conditions(user=None):
    """
    List view filter for Project Installation
    ...
    if is_vendor_manager:
        return f"""
            `tabProject Installation`.name IN ({doc_list})
            AND `tabProject Installation`.assigned_vendor = {frappe.db.escape(supplier)}
        """
