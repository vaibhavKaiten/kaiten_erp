...
"""
Meter Installation – List View Visibility (Fresh & Strict)
...
def get_permission_query_conditions(user=None):
    """
    List view filter for Meter Installation
    ...
    if not (is_vendor_manager or is_vendor_executive):
        return "`tabMeter Installation`.name = ''"
    ...
    todos = frappe.db.sql(
        ...
        FROM `tabToDo`
        WHERE reference_type = 'Meter Installation'
          AND allocated_to = %s
    ...
    if is_vendor_manager:
        return f"""
            `tabMeter Installation`.name IN ({doc_list})
            AND `tabMeter Installation`.assigned_vendor = {frappe.db.escape(supplier)}
        """
